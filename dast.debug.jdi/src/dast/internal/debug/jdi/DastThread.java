...
public class DastThread extends JDIThreadAdapter implements IJavaThread, IStep{

	private DastStepHandler stepHandler;

	  DastThread(final JDIDebugTarget target, final ThreadReference thread)
	  ...
	  public boolean canResume()
	  {
	     ...
	     * event in the trace.
	     */
	    //final IExecutionModel model = getDastDebugTarget().model();
	  ...
	  @Override
	  public DastDebugTarget getDebugTarget()
	  {
	    return (DastDebugTarget) super.getDebugTarget();
	  }
	  ...
	  {
		  /*!getDastDebugTarget().isManualStart() || */
	    /*if (getDastDebugTarget().isActive())
	    {*/
	      createRequests();
	    //}
	    super.initialize();
	  ...
	  private DastDebugTarget getDastDebugTarget()
	  {
	    return (DastDebugTarget) super.getDebugTarget();
	  }
	  ...
	  {
	    return getDastDebugTarget().isActive();
	  }
	 ...
	 @Override
	  ...
	  void createRequests()
	  {
	    if (stepHandler == null)
	    {
	      this.stepHandler = new DastStepHandler();
	    }
	   ...
	   * Helper class to perform stepping on a thread.
	   */
	  protected class DastStepHandler implements IJDIEventListener
	  {
	    //private final IJDIEventHandler jdiHandler;
	    private StepRequest stepRequest;

	    protected DastStepHandler()
	    {
	      //jdiHandler = ((DastDebugTarget) getDebugTarget()).jdiHandler();
	    }
	      ...
	      if (stepHandler != null)
	      {
	        switch (stepHandler.getStepKind())
	        {
	          case StepRequest.STEP_INTO:
	            ...
	            {
	              return stepHandler.handleEvent(event, target, suspendVote, eventSet);
	            }
	            break;
		...
		protected void createRequest()
	    {
	      ...
	      if (manager != null)
	      {
	        try
	        {
	          ...
	          final DastDebugTarget target = (DastDebugTarget) getDebugTarget();
	          /*final IModelFilter requestFilter = target.jdiManager().modelFilter();
	          requestFilter.filter(stepRequest);
	          stepRequest.setSuspendPolicy(target.generateLockEvents() ? EventRequest.SUSPEND_ALL
	  ...
	  {
	    return DastDebugPlugin.getDefault() != null ? DastDebugPlugin.getDefault().jdiManager(this)
	        : null;
	  }*/
}
